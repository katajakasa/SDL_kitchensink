...
KIT_LOCAL int Kit_GetSubtitleDecoderSDLTextureInfo(const Kit_Decoder *dec, SDL_Rect *sources, SDL_Rect *targets, int limit);
KIT_LOCAL int Kit_GetSubtitleDecoderOutputFormat(const Kit_Decoder *dec, Kit_SubtitleOutputFormat *output);
KIT_LOCAL int Kit_GetSubtitleDecoderRawFrames(const Kit_Decoder *dec, unsigned char ***items, SDL_Rect **sources, SDL_Rect **targets, double sync_ts);
